    ...
    #endif
{
    ...
    {
        // Allocate minimal buffer for empty string
        m_ichMac = 0;
        AdjustMaxArraySize( 1 );
        g_ScroobyMemoryCount += m_ichMax;
        m_szString = new char[m_ichMax];
        m_szString[0] = '\0';
    }
...
PascalCString PascalCString::JustFilename()
{
    ...
    // Calculate filename length safely
    int filenameLen = Length() - (index + 1);
    if( filenameLen <= 0 )
    {
        return PascalCString( "" );
    }
    // Allocate proper buffer size instead of fixed 20 bytes
    radMemoryAllocator old = ::radMemorySetCurrentAllocator( RADMEMORY_ALLOC_TEMP );
    char* temp = new char[filenameLen + 1];
    ::radMemorySetCurrentAllocator( old );
