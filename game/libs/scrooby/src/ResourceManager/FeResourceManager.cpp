    ...
    for( i = 0; i < mResources.Size(); i++ )
    {
        ...
        if( resource )
        {
            ...
            else if( (resource->m_Status == RM_P3D_LOAD_IN_PROGRESS) || (resource->m_Status == RM_NOT_LOADED) )
            {
                ...
                {
                    searchName = resource->GetName();
                    entity = p3d::find<tEntity>( resource->GetName() );
                }
                else
                {
                    searchName = mResources[i]->m_inventoryName;
                    entity = p3d::find<tEntity>( mResources[i]->m_inventoryName );
                }
                if( entity == NULL )
                {
                ...
                if( entity != NULL )
                {
                    ...
                    if( mResources[i]->m_type == RT_PROJECT )
                    {
                        ...
                        // If project has no screens (lastScreen <= project index), add callback immediately
                        int lastScreen = m_pLoadingProject->GetLastScreenToLoad();
                        if( lastScreen <= i )
                        {
                            radMemoryAllocator old2 = ::radMemorySetCurrentAllocator( RADMEMORY_ALLOC_TEMP );
                                mCallbackID = AddResource( "ProjectCallback", NULL, RT_PROJECT_LOAD_CALLBACK );
                            ::radMemorySetCurrentAllocator( old2 );
                        }
                    }
