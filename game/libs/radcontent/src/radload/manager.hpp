...
public:
    ...
    {
    public:
        // Note: expects options.filename to be a malloc'd copy that we take ownership of
        QueueItem( radLoadOptions& options ) : m_options( options ), m_ownFilename( options.filename != 0 )
        {
        }
        ~QueueItem()
        {
            // Free the filename we own (was malloc'd in radLoadManager::Load)
            if( m_ownFilename && m_options.filename )
            {
                free( m_options.filename );
            }
            m_options.filename = 0;
        }
