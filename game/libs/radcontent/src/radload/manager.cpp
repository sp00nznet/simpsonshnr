...
void radLoadManager::Load( radLoadOptions* options, radLoadRequest** request )
{
    ...
    // Deep copy the filename BEFORE creating QueueItem to ensure it survives
    radLoadOptions optionsCopy = *options;
    if( options->filename )
    {
        int len = strlen( options->filename );
        optionsCopy.filename = (char*)malloc( len + 1 );
        if( optionsCopy.filename )
        {
            strcpy( optionsCopy.filename, options->filename );
        }
    }
...
void radLoadManager::Load( const char* filename, radLoadRequest** request )
{
    ...
    options.filename = new char[strlen( filename ) + 1];  // +1 for null terminator
