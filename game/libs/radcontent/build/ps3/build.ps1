# radcontent (radload) PS3 Build Script

$ErrorActionPreference = "Continue"

# Base paths (absolute)
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RadcontentDir = (Resolve-Path "$ScriptDir\..\..").Path
$LibsDir = (Resolve-Path "$RadcontentDir\..").Path
$SourceDir = "$RadcontentDir\src"
$ObjDir = "$ScriptDir\Debug"
$LibOutDir = "$RadcontentDir\lib"
...
Write-Host "Building radcontent for PS3"
...
if (!(Test-Path "$ObjDir\radload")) {
    New-Item -ItemType Directory -Path "$ObjDir\radload" -Force | Out-Null
}
...
$LibFile = "$LibOutDir\radcontentp3d.a"
