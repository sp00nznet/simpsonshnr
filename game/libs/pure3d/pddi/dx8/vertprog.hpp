...
// #define VSH_TEST  // Disabled - requires pre-compiled shader headers
...
public:
    ...
    unsigned GetD3DVS(void)
    {
        ...
        {
            HRESULT hr = context->GetDisplay()->GetD3DDevice()->CreateVertexShader(decl, tokens, (DWORD*)&vertexShader, usage);
            if (FAILED(hr))
            {
                // If vertex shader creation failed, try without tokens (fixed-function fallback)
                context->GetDisplay()->GetD3DDevice()->CreateVertexShader(decl, NULL, (DWORD*)&vertexShader, usage);
            }
        }
        return vertexShader;
    }
