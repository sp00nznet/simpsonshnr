...
void DDVERIFY(HRESULT res)
{
    ...
    // On modern Windows with dgVoodoo, many D3D8 "errors" are not actually fatal
    // Log them but try to continue anyway
    char errStr[256];
    D3DXGetErrorString(res, errStr, 256);

    char errBuf[512];
    wsprintf(errBuf, "DDVERIFY: %s (0x%08X) - continuing anyway\n", errStr, res);
    OutputDebugString(errBuf);

    // Don't show message boxes or exit - just continue and hope for the best
    // The game often works fine even with these errors on modern systems
}
