...
void d3dPrimBuffer::Display()
{
    ...
    {
        HRESULT hr;
        hr = d3d->SetIndices(indices, 0);
        if(FAILED(hr) && showDebug) { OutputDebugStringA("[PRIM] SetIndices FAILED\n"); }

        unsigned vs = vertexProgram->GetD3DVS();
        hr = d3d->SetVertexShader(vs);
        if(FAILED(hr) && showDebug) {
            char msg[128];
            sprintf(msg, "[PRIM] SetVertexShader(%u) FAILED hr=0x%x\n", vs, hr);
            OutputDebugStringA(msg);
        }

        hr = d3d->SetStreamSource(0, buffer, vertexProgram->GetStride());
        if(FAILED(hr) && showDebug) { OutputDebugStringA("[PRIM] SetStreamSource FAILED\n"); }

        HRESULT res = d3d->DrawIndexedPrimitive(primitiveType, 0, maxVertex, 0, VertsToPrims(primTypePDDI, nIndex));
        if(FAILED(res) && showDebug) {
            char msg[128];
            sprintf(msg, "[PRIM] DrawIndexedPrimitive FAILED hr=0x%x\n", res);
            OutputDebugStringA(msg);
        }
    }
    else
    {
        ...
        if(FAILED(res) && showDebug) {
            char msg[128];
            sprintf(msg, "[PRIM] DrawPrimitive FAILED hr=0x%x\n", res);
            OutputDebugStringA(msg);
        }
    }
