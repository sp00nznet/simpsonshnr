...
#if defined(LINUX) || defined(RAD_PS3)
...
{
    // NOTE: This runs on the LOAD THREAD. Do NOT use printf here - it calls
    // malloc internally which corrupts dlmalloc heap state on PS3.
...
void tLoadRequest::Complete()
{
    if( request )
    {
        ...
        {
            radLoadService();
            radFileService();  // Service the radFile system to process drive requests
        }
    }
}
