...
{
#ifdef RAD_PS3
    unsigned filePos = realFile->GetPosition();
    unsigned chunkStart = chunkStack[stackTop].startPosition;
    unsigned chunkLen = chunkStack[stackTop].chunkLength;
    if (filePos > chunkStart + chunkLen) {
        // Skip the assertion for now to continue debugging
        return;
    }
#endif
