...
public:
    ...
    virtual void SetExpectedFileSize(const int size) { expectedFileSize = size; }
    virtual int  GetExpectedFileSize()               { return expectedFileSize; }
    virtual void SetExpectedDimensions( int w, int h ) { expectedWidth = w; expectedHeight = h; }
    virtual int  GetExpectedWidth()                    { return expectedWidth; }
    virtual int  GetExpectedHeight()                   { return expectedHeight; }
...
TextureBuilder::TextureBuilder() :
    ...
    alphaDepth(8), nMip(0), type(PDDI_TEXTYPE_RGB), usage(PDDI_USAGE_STATIC), desiredDepth(0), expectedFileSize(-1),
...
bool TextureBuilder::BeginImage(int width, int height, int bpp, tImageHandler::Builder::Origin origin, pddiColour* p)
{
    if (texture == NULL)
    {
        ...
        {
#ifdef RAD_PS3
             // PS3/PSGL has no native DXT support. LoadDXTNFilePS3() software-decompresses
             // DXT to 32bpp ARGB, so allocate a 32bpp RGB texture to hold the result.
             texture->Create(width, height, 32, 8, nMip, PDDI_TEXTYPE_RGB, usage);
#else
...
#if defined(WIN32) || defined(RAD_GAMECUBE) || defined(RAD_PS3)
...
tTexture* tImageFactory::ParseAsTexture( tFile* file, char* inventoryName, const int size, tImageHandler::Format format)
{
    ...
    if (imgWidth > 0 && imgHeight > 0)
        builder.SetExpectedDimensions(imgWidth, imgHeight);
    handler->CreateImage(file, &builder);
    tTexture* texture = builder.GetTexture();
