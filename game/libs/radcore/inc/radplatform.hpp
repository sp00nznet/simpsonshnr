...
#if !defined(RAD_GAMECUBE) && !defined(RAD_PS2) && !defined(RAD_PS3) && !defined(RAD_XBOX) && !defined(RAD_WIN32)
    #error 'FTech requires definition of RAD_GAMECUBE, RAD_PS2, RAD_PS3, RAD_XBOX, or RAD_WIN32'
#endif
...
#ifdef RAD_PS3
//
// PS3 platform initialization - simple like GameCube since no IOP concept.
// For compatibility, we also provide the PS2-style enum constants.
//
enum radPlatformIOPMedia
{
    IOPMediaHost,           // Load from host
    IOPMediaCDVD            // Load from disc
};

enum radPlatformGameMediaType
{
    GameMediaCD,            // Game is distributed on CD
    GameMediaDVD,           // Game is distributed on DVD/Blu-ray
    GameMediaBluRay = GameMediaDVD  // Alias for clarity
};

// PS2-style signature for compatibility
void radPlatformInitialize( const char* pIrxPath, radPlatformIOPMedia IOPMedia,
                            radPlatformGameMediaType mediaType = GameMediaDVD,
                            const char* pIopImage = NULL,
                            radMemoryAllocator = RADMEMORY_ALLOC_DEFAULT );
...
//
// PS3 Platform interface.
//
struct IRadPlatform : public IRefCount
{
    virtual radPlatformGameMediaType GetMediaType( void ) = 0;
};
