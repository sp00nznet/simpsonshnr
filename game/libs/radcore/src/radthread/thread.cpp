...
#include "pch.hpp"
    ...
    #include <sys/timer.h>
    #include <sys/tty.h>
static void _RTPrint(const char* msg)
{
...
)
{
...
#ifdef RAD_PS3
    //
    // PS3 sleep implementation using sys_timer_usleep
    //
    if( milliseconds == 0 )
    {
    ...
    {
        // Convert milliseconds to microseconds for sys_timer_usleep
        ...
        m_pActiveFiber( &m_Fiber )
{
...
#ifdef RAD_PS3
    //
    // PS3 thread creation using sys_ppu_thread_create
    //
    int ps3Priority = 1001; // Default priority (middle range)
    switch(priority)
    {
        case PriorityIdle:        ps3Priority = 2000; break;
        case PriorityBelowNormal: ps3Priority = 1500; break;
        case PriorityNormal:      ps3Priority = 1001; break;
        case PriorityAboveNormal: ps3Priority = 750;  break;
        case PriorityHigh:        ps3Priority = 500;  break;
    }

    _RTPrint("[RT] sys_ppu_thread_create call\n");
    int ret = sys_ppu_thread_create(
        &m_ThreadId,                    // Thread ID output
        PS3ThreadWrapper,               // Entry function (PS3-compatible wrapper)
        (uint64_t)this,                 // Argument
        ps3Priority,                    // Priority (0-3071, lower = higher priority)
        stackSize,                      // Stack size
        SYS_PPU_THREAD_CREATE_JOINABLE, // Flags
        "radThread"                     // Thread name
    );
    {
        char b[64]; int i = 0; const char* a = "[RT] sys_ppu_thread_create ret=";
        while (a[i]) { b[i] = a[i]; ++i; }
        int n = ret; if (n < 0) { b[i++] = '-'; n = -n; }
        char num[12]; int k = 0;
        if (n == 0) num[k++] = '0';
    ...
    if(ret != 0)
    {
        rDebugPrintf("PS3 thread creation failed: %d\n", ret);
        m_IsRunning = false;
    }
#endif
...
//=============================================================================
// Function:    radThread::PS3ThreadWrapper (PS3 only)
//=============================================================================
// Description: PS3-specific wrapper that matches the sys_ppu_thread signature
//              and calls the real InternalThreadEntry
//------------------------------------------------------------------------------

#ifdef RAD_PS3
void radThread::PS3ThreadWrapper( uint64_t param )
{
    _RTPrint("[RT] PS3ThreadWrapper enter\n");
    radThread* pThread = (radThread*)param;

    pThread->InternalThreadEntry( (void*)param );
    _RTPrint("[RT] PS3ThreadWrapper exit\n");
...
)
{
     ...
     {
         rDebugPrintf("[radThread] WARNING: SetLocalStorage failed - GetActiveThread returned NULL!\n");
     }
}
...
)
{
     ...
     rDebugPrintf("[radThread] WARNING: GetLocalStorage failed - GetActiveThread returned NULL!\n");
     return( NULL );
}
