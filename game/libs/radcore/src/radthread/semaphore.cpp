...
#ifdef RAD_PS3
    //
    // PS3 uses sys_semaphore_create
    //
    sys_semaphore_attribute_t attr;
    sys_semaphore_attribute_initialize(attr);
    int ret = sys_semaphore_create(&m_Semaphore, &attr, count, 32000);
    rAssert(ret == CELL_OK);
#endif
...
#ifdef RAD_PS3
    //
    // PS3: destroy the semaphore
    //
    sys_semaphore_destroy(m_Semaphore);
#endif
...
#ifdef RAD_PS3
    //
    // PS3: wait on the semaphore (blocks until count > 0)
    //
    sys_semaphore_wait(m_Semaphore, 0);  // 0 = wait forever
#endif
}
...
#ifdef RAD_PS3
    //
    // PS3: signal the semaphore (increments count)
    //
