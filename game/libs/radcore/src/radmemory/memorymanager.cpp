    ...
    radMemoryAllocator parent, IRadMemoryAllocator* pIRadMemoryAllocator )
{
    ...
    // On PS3, skip re-registration if allocator is already in use
    // This can happen during game restart flows
    if ( g_AllocatorTreeNodes[ allocator ].m_pIRadMemoryAllocator != g_pRadMemoryAllocator_Malloc )
    {
#ifdef RAD_PS3
        // Already registered - skip silently on PS3
        return;
#else
        rAssert( false && "Allocator already registered" );
#endif
    }
