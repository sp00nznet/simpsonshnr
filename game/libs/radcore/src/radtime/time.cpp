...
//
// Under RAD_PS3, use system time
#ifdef RAD_PS3
static uint64_t s_PS3StartTime = 0;
#endif
...
void radTimeInitialize(  )
{
    ...
    if( s_InitializedCount == 1 )
    {
        ...
        //
        // On the RAD_PS3, record the start time
        //
        #ifdef RAD_PS3
            s_PS3StartTime = sys_time_get_system_time();
        #endif // RAD_PS3
    }
}
...
unsigned int radTimeGetMicroseconds( void )
{
    ...
    #ifdef RAD_PS3
        //
        // On PS3, use system time (returns time in microseconds)
        //
        uint64_t currentTime = sys_time_get_system_time();
        return (unsigned int)(currentTime - s_PS3StartTime);
    #endif // RAD_PS3
}
...
radTime64 radTimeGetMicroseconds64( void )
{
    ...
    #ifdef RAD_PS3
        //
        // On PS3, use system time
        //
        uint64_t currentTime = sys_time_get_system_time();
        return (radTime64)(currentTime - s_PS3StartTime);
    #endif // RAD_PS3
}
...
unsigned int radTimeGetMilliseconds( void )
{
    ...
    #ifdef RAD_PS3
        //
        // On PS3, convert microseconds to milliseconds
        //
        uint64_t currentTime = sys_time_get_system_time();
        return (unsigned int)((currentTime - s_PS3StartTime) / 1000);
    #endif // RAD_PS3
}
...
unsigned int radTimeGetSeconds( void )
{
    ...
    #ifdef RAD_PS3
        //
        // On PS3, convert microseconds to seconds
        //
        uint64_t currentTime = sys_time_get_system_time();
        return (unsigned int)((currentTime - s_PS3StartTime) / 1000000);
    #endif // RAD_PS3
}
...
void radTimeGetDate( radDate* pDate )
{
    ...
    //
    // On PS3, use cell RTC functions
    //
    CellRtcDateTime dateTime;
    cellRtcGetCurrentClockLocalTime(&dateTime);

    pDate->m_Year = dateTime.year;
    pDate->m_Month = (unsigned char)dateTime.month;
    pDate->m_Day = (unsigned char)dateTime.day;
    pDate->m_Hour = (unsigned char)dateTime.hour;
    pDate->m_Minute = (unsigned char)dateTime.minute;
    pDate->m_Second = (unsigned char)dateTime.second;

#else
    #error 'FTech requires definition of RAD_GAMECUBE, RAD_PS2, RAD_XBOX, RAD_WIN32, or RAD_PS3'
#endif
