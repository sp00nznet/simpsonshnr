...
#ifdef RAD_PS3
#include "../ps3/ps3bddrive.hpp"
#include "../ps3/ps3savedrive.hpp"
#endif
...
#ifdef RAD_PS3
    strcpy( driveSpec, s_PS3BDDriveName );
#endif // RAD_PS3
...
#ifdef RAD_PS3
    if ( strcmp( driveSpec, s_PS3BDDriveName ) == 0 )
    {
        return true;
    }
    else if ( radStringMatchesWildCardPattern( driveSpec, s_PS3SaveDrive ) )
    {
        int slot = ( int )( driveSpec[ PS3SAVEDRIVE_SLOT_LOC ] - '0' );
        if ( slot < 0 || slot > 9 )
        {
            return false;
        }
...
void PlatformDrivesFactory( radDrive** ppDrive, const char* driveSpec, radMemoryAllocator alloc )
{
    ...
    if ( strcmp( driveSpec, s_PS3BDDriveName ) == 0 )
    {
        radPs3BdDriveFactory( ppDrive, driveSpec, alloc );
    }
    else
    {
        radPs3SaveDriveFactory( ppDrive, driveSpec, alloc );
    }
#endif // RAD_PS3
