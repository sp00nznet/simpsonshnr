...
inline float SuperCamController::GetAxisValue( unsigned int buttonId ) const
{
    ...
    case stickX:
        {
            // Check for direct axis value first (from XInput RightStickX mapping)
            float direct = GetValue( stickX );
            if ( direct != 0.0f )
            {
                return direct;
            }
            // Fall back to keyboard-style button combination
    ...
    case stickY:
        {
            // Check for direct axis value first (from XInput RightStickY mapping)
            float direct = GetValue( stickY );
            if ( direct != 0.0f )
            {
                return direct;
            }
