...
void SuperCamController::LoadControllerMappings( unsigned int controllerId )
{
    ...
    #ifdef RAD_WIN32
    ClearMap(0);
    // Direct right stick axis mapping (for XInput controllers)
    // These take priority - provides -1 to 1 values like console versions
    Map( "RightStickX", stickX, 0, controllerId );
    Map( "RightStickY", stickY, 0, controllerId );
    // Keyboard/button-style mappings (combined by GetAxisValue)
