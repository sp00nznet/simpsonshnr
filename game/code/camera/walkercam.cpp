...
void WalkerCam::UpdatePositionNormal( unsigned int milliseconds, float timeMod )
{
    ...
    {
#ifdef RAD_WIN32
        // Read button values directly like FollowCam does for vehicle camera
        float leftX = mController->GetValue( SuperCamController::stickXdown );
        float rightX = mController->GetValue( SuperCamController::stickXup );
        float xAxis = ( rightX > leftX ) ? rightX : -leftX;

        float upY = mController->GetValue( SuperCamController::stickYup );
        float downY = mController->GetValue( SuperCamController::stickYdown );
        float zAxis = ( upY > downY ) ? upY : -downY;

        // Debug output for WalkerCam input - write to file for visibility
        static int walkerDebugCounter = 0;
        static FILE* walkerDebugFile = NULL;
        static bool walkerFileInitDone = false;
        if (!walkerFileInitDone)
        {
            walkerFileInitDone = true;
            walkerDebugFile = fopen("C:\\walkercam_debug.txt", "w");
            if (walkerDebugFile) { fprintf(walkerDebugFile, "WalkerCam debug started\n"); fflush(walkerDebugFile); }
        }
        // Always log every 60 frames to verify WalkerCam is active
        if (walkerDebugCounter++ % 60 == 0)
        {
            char debugMsg[256];
            sprintf(debugMsg, "WalkerCam[%d]: leftX=%.3f rightX=%.3f xAxis=%.3f | upY=%.3f downY=%.3f zAxis=%.3f\n",
                walkerDebugCounter, leftX, rightX, xAxis, upY, downY, zAxis);
            OutputDebugStringA(debugMsg);
            if (walkerDebugFile) { fprintf(walkerDebugFile, "%s", debugMsg); fflush(walkerDebugFile); }
        }
#else
...
void WalkerCam::UpdatePositionInCollision( unsigned int milliseconds, float timeMod )
{
    ...
    float leftX = mController->GetValue( SuperCamController::stickXdown );
    float rightX = mController->GetValue( SuperCamController::stickXup );
    mXAxis = ( rightX > leftX ) ? rightX : -leftX;
#else
