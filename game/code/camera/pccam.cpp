...
PCCam::PCCam() :
    ...
    mFOVDelta( 0.0f ),
    mLastPCCamFacing( 0 ),
    m_fSmoothedStickX( 0.0f ),
    m_fSmoothedStickY( 0.0f ),
...
void PCCam::Update( unsigned int milliseconds )
{
    ...
    if ( mController )
    {
        ...
        // Also check for gamepad right stick Y-axis input with smoothing
        float rawStickY = mController->GetAxisValue( SuperCamController::stickY );
        float rawStickX = mController->GetAxisValue( SuperCamController::stickX );

        // Smoothing: lerp toward target value each frame
        // STICK_SMOOTH_RATE controls responsiveness (higher = faster response, max 1.0)
        // Multiply by timeMod to be frame-rate independent
        static float STICK_SMOOTH_RATE = 0.5f;
        float smoothFactor = rmt::Min(1.0f, STICK_SMOOTH_RATE * timeMod);

        // Lerp toward the target value for smooth input
        m_fSmoothedStickX = m_fSmoothedStickX + (rawStickX - m_fSmoothedStickX) * smoothFactor;
        m_fSmoothedStickY = m_fSmoothedStickY + (rawStickY - m_fSmoothedStickY) * smoothFactor;

        // Apply small deadzone to smoothed values to prevent drift
        static float SMOOTHED_DEADZONE = 0.02f;
        float stickX = (rmt::Fabs(m_fSmoothedStickX) < SMOOTHED_DEADZONE) ? 0.0f : m_fSmoothedStickX;
        float stickY = (rmt::Fabs(m_fSmoothedStickY) < SMOOTHED_DEADZONE) ? 0.0f : m_fSmoothedStickY;

        if ( stickY != 0.0f )
        {
            // Scale stick input - higher value = more responsive
            static float STICK_Y_SCALE = 0.8f;
            yAxis += stickY * STICK_Y_SCALE;
        }

        m_fYawVelocity += yAxis;

        // Also add gamepad right stick X-axis for horizontal rotation (pitch velocity)
        if ( stickX != 0.0f )
        {
            // Scale stick input - higher value = more responsive
            static float STICK_X_SCALE = 0.3f;
            m_fPitchVelocity += stickX * STICK_X_SCALE;
        }

        // Debug output for PC_CAM input
        static int pccamDebugCounter = 0;
        if (pccamDebugCounter++ % 60 == 0)
        {
            char debugMsg[256];
            sprintf(debugMsg, "PCCam[%d]: rawX=%.3f rawY=%.3f smoothX=%.3f smoothY=%.3f\n",
                pccamDebugCounter, rawStickX, rawStickY, stickX, stickY);
