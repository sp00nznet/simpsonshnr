...
{
#ifdef RAD_PS3
    { unsigned int w; sys_tty_write(0,"  GDM: entering CreateInstance\n",31,&w); }
#endif
MEMTRACK_PUSH_GROUP( "GameDataManager" );
#ifdef RAD_PS3
    { unsigned int w; sys_tty_write(0,"  GDM: about to new GameDataManager\n",36,&w); }
#endif
    spInstance = new GameDataManager;
#ifdef RAD_PS3
    { unsigned int w; sys_tty_write(0,"  GDM: new done\n",16,&w); }
#endif
    rAssert( spInstance != NULL );
MEMTRACK_POP_GROUP( "GameDataManager" );

#ifdef RAD_PS3
    { unsigned int w; sys_tty_write(0,"  GDM: about to create MemCardMgr\n",34,&w); }
#endif
...
MEMTRACK_PUSH_GROUP( "GameDataManager" );
    ...
    //
#ifdef RAD_PS3
    // Skip memory card init for now - PS3 save drive causes hang
    // Skip memory card init for now - PS3 save drive causes hang
#else
