...
#ifdef RAD_PS3
#include <sys/tty.h>
static void _LMPrint(const char* a, const char* b)
{
    char buf[512];
    int i = 0;
...
#define LM_TRACE(a, b) _LMPrint(a, b)
#else
#define LM_TRACE(a, b) do {} while(0)
#endif
...
void LoadingManager::AddCallback( LoadingManager::ProcessRequestsCallback* pCallback, void* pUserData)
{
    ...
    {
        LM_TRACE("[LM] AddCallback immediate fire (queue empty): ", "");
        pCallback->OnProcessRequestsComplete(pUserData);
        return;
    }
    LM_TRACE("[LM] AddCallback queued (will fire after pending): ", "");
...
void LoadingManager::OnLoadFileComplete( void* pUserData )
{
    ...
    LM_TRACE("[LM] DONE load: ", request.filename);
    ...
    if(!mLoading)
    {
        if(mRequestHead != mRequestTail)
        {
            ...
            if ( request.filename[0] != '\0' )
            {
                ...
                mLoading = true;
                LM_TRACE("[LM] START load: ", request.filename);
            ...
            else
            {
                ...
                LM_TRACE("[LM] FENCE callback firing (queue drained)", "");
                mRequestHead = (mRequestHead + 1) % MAX_REQUESTS;
                mLoading = false;
