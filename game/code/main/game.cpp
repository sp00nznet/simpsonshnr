...
#ifdef RAD_PS3
#include <sys/tty.h>
static void _GamePrint(const char* msg)
{
    unsigned int len = 0;
    const char* p = msg;
    while (*p++) len++;
...
static void _GamePrintHB(unsigned int frame, int ctx)
{
    char buf[64];
    const char* prefix = "[HB] frame=";
    int i = 0;
    while (prefix[i]) { buf[i] = prefix[i]; ++i; }
    char num[12]; int n = 0;
    if (frame == 0) { num[n++] = '0'; }
    else { unsigned int v = frame; while (v) { num[n++] = '0' + (v % 10); v /= 10; } }
    while (n > 0) { buf[i++] = num[--n]; }
    const char* mid = " ctx=";
    int j = 0; while (mid[j]) { buf[i++] = mid[j++]; }
    int c = ctx; if (c < 0) { buf[i++] = '-'; c = -c; }
    n = 0;
    if (c == 0) { num[n++] = '0'; }
    else { while (c) { num[n++] = '0' + (c % 10); c /= 10; } }
    while (n > 0) { buf[i++] = num[--n]; }
    buf[i++] = '\n'; buf[i] = 0;
    _GamePrint(buf);
}
...
#ifdef RAD_PS3
        ++hb_frame;
        if (hb_frame == 1 || hb_frame == 10 || hb_frame == 60 || (hb_frame % 300) == 0)
        {
            int ctx = mpGameFlow ? (int)mpGameFlow->GetCurrentContext() : -1;
            _GamePrintHB(hb_frame, ctx);
        }
#endif
