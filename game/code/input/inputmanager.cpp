...
#ifdef RAD_WIN32
// XInput controllers for modern Xbox gamepad support
static XInputController s_xinputControllers[4];
static bool s_xinputAvailable[4] = { false, false, false, false };
#endif
...
#ifdef RAD_WIN32
    // Initialize XInput for modern Xbox controller support
    XInputController::Initialize();

    // Check which XInput controllers are available
    for (int i = 0; i < 4; i++)
    {
        s_xinputAvailable[i] = XInputController::IsConnected(i);
        if (s_xinputAvailable[i])
        {
            rDebugPrintf("XInput controller %d detected\n", i);
        }
    }
#endif
...
#ifdef RAD_WIN32
    // Poll XInput controllers for modern Xbox gamepad support
    for (int xi = 0; xi < 4; xi++)
    {
        bool wasConnected = s_xinputAvailable[xi];
        s_xinputAvailable[xi] = s_xinputControllers[xi].Poll(xi);

        // Detect connection changes
        if (s_xinputAvailable[xi] != wasConnected)
        {
            if (s_xinputAvailable[xi])
            {
                rDebugPrintf("XInput controller %d connected\n", xi);
            }
            else
            {
                rDebugPrintf("XInput controller %d disconnected\n", xi);
            }
        }
    }
#endif
...
InputManager::~InputManager()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        s_xinputControllers[i].StopVibration();
    }
    XInputController::Shutdown();
...
//=============================================================================
// XInput accessor functions for modern Xbox controller support
//=============================================================================

bool InputManager::IsXInputConnected(int port) const
{
    if (port < 0 || port > 3)
        return false;
    return s_xinputAvailable[port];
}

float InputManager::GetXInputLeftStickX(int port) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return 0.0f;
    return s_xinputControllers[port].GetLeftStickX();
}

float InputManager::GetXInputLeftStickY(int port) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return 0.0f;
    return s_xinputControllers[port].GetLeftStickY();
}

float InputManager::GetXInputRightStickX(int port) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return 0.0f;
    return s_xinputControllers[port].GetRightStickX();
}

float InputManager::GetXInputRightStickY(int port) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return 0.0f;
    return s_xinputControllers[port].GetRightStickY();
}

float InputManager::GetXInputLeftTrigger(int port) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return 0.0f;
    return s_xinputControllers[port].GetLeftTrigger();
}

float InputManager::GetXInputRightTrigger(int port) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return 0.0f;
    return s_xinputControllers[port].GetRightTrigger();
}

bool InputManager::IsXInputButtonPressed(int port, unsigned short button) const
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return false;
    return s_xinputControllers[port].IsButtonPressed(button);
}

void InputManager::SetXInputVibration(int port, float leftMotor, float rightMotor)
{
    if (port < 0 || port > 3 || !s_xinputAvailable[port])
        return;
    s_xinputControllers[port].SetVibration(leftMotor, rightMotor);
}
