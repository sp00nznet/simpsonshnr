...
void UserController::Update( unsigned timeins )
{
    ...
    // XInput controller support for modern Xbox controllers
    InputManager* pInputMgr = InputManager::GetInstance();
    if(pInputMgr && m_controllerId < 4 && pInputMgr->IsXInputConnected(m_controllerId))
    {
        // Map XInput to game buttons
        // Movement buttons (virtual buttons 0-3 are MoveUp, MoveDown, MoveLeft, MoveRight)
        float leftX = pInputMgr->GetXInputLeftStickX(m_controllerId);
        float leftY = pInputMgr->GetXInputLeftStickY(m_controllerId);

        // Left stick for movement
        SetButtonValue(InputManager::MoveUp, leftY > 0.3f ? leftY : 0.0f, false);
        SetButtonValue(InputManager::MoveDown, leftY < -0.3f ? -leftY : 0.0f, false);
        SetButtonValue(InputManager::MoveLeft, leftX < -0.3f ? -leftX : 0.0f, false);
        SetButtonValue(InputManager::MoveRight, leftX > 0.3f ? leftX : 0.0f, false);

        // Face buttons
        SetButtonValue(InputManager::Attack, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_X) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::Jump, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_A) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::Sprint, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_B) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::DoAction, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_Y) ? 1.0f : 0.0f, false);

        // Driving controls - Right trigger for accelerate, Left trigger for brake
        SetButtonValue(InputManager::Accelerate, pInputMgr->GetXInputRightTrigger(m_controllerId), false);
        SetButtonValue(InputManager::Reverse, pInputMgr->GetXInputLeftTrigger(m_controllerId), false);

        // Steering uses left stick
        SetButtonValue(InputManager::SteerLeft, leftX < -0.1f ? -leftX : 0.0f, false);
        SetButtonValue(InputManager::SteerRight, leftX > 0.1f ? leftX : 0.0f, false);

        // Other buttons
        SetButtonValue(InputManager::HandBrake, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_RIGHT_SHOULDER) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::GetOutCar, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_Y) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::Horn, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_LEFT_THUMB) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::ResetCar, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_RIGHT_THUMB) ? 1.0f : 0.0f, false);

        // Camera controls - right stick (for both on-foot and vehicle cameras)
        float rightX = pInputMgr->GetXInputRightStickX(m_controllerId);
        float rightY = pInputMgr->GetXInputRightStickY(m_controllerId);

        // Apply deadzone for right stick
        float rightXDeadzone = 0.2f;
        float rightYDeadzone = 0.2f;
        if (rightX > -rightXDeadzone && rightX < rightXDeadzone) rightX = 0.0f;
        if (rightY > -rightYDeadzone && rightY < rightYDeadzone) rightY = 0.0f;

        // Debug output for right stick - write to file for visibility
        static int debugCounter = 0;
        static FILE* debugFile = NULL;
        static bool fileInitDone = false;
        if (!fileInitDone)
        {
            fileInitDone = true;
            debugFile = fopen("C:\\camera_debug.txt", "w");
            if (debugFile) { fprintf(debugFile, "XInput camera debug started\n"); fflush(debugFile); }
        }
        if ((rightX != 0.0f || rightY != 0.0f) && (debugCounter++ % 30 == 0))
        {
            char debugMsg[256];
            sprintf(debugMsg, "XInput RightStick: X=%.3f Y=%.3f CamLeft=%.3f CamRight=%.3f\n",
                rightX, rightY,
                rightX < 0.0f ? -rightX : 0.0f,
                rightX > 0.0f ? rightX : 0.0f);
            OutputDebugStringA(debugMsg);
            if (debugFile) { fprintf(debugFile, "%s", debugMsg); fflush(debugFile); }
        }

        // Set direct axis values for right stick (for SuperCamController direct mapping)
        // These provide -1 to 1 values like the console versions
        SetButtonValue(InputManager::RightStickX, rightX, false);
        SetButtonValue(InputManager::RightStickY, rightY, false);

        // On-foot camera - use absolute values since button system expects 0-1
        SetButtonValue(InputManager::CameraLeft, rightX < 0.0f ? -rightX : 0.0f, false);
        SetButtonValue(InputManager::CameraRight, rightX > 0.0f ? rightX : 0.0f, false);
        SetButtonValue(InputManager::CameraMoveIn, rightY > 0.0f ? rightY : 0.0f, false);
        SetButtonValue(InputManager::CameraMoveOut, rightY < 0.0f ? -rightY : 0.0f, false);
        // Vehicle camera
        SetButtonValue(InputManager::CameraCarLeft, rightX < 0.0f ? -rightX : 0.0f, false);
        SetButtonValue(InputManager::CameraCarRight, rightX > 0.0f ? rightX : 0.0f, false);
        SetButtonValue(InputManager::CameraCarLookUp, rightY > 0.0f ? rightY : 0.0f, false);
        SetButtonValue(InputManager::CameraCarLookBack, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_LEFT_SHOULDER) ? 1.0f : 0.0f, false);

        // Menu/FE buttons
        SetButtonValue(InputManager::feSelect, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_A) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::feBack, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_B) ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::feMoveUp, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_DPAD_UP) || leftY > 0.5f ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::feMoveDown, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_DPAD_DOWN) || leftY < -0.5f ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::feMoveLeft, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_DPAD_LEFT) || leftX < -0.5f ? 1.0f : 0.0f, false);
        SetButtonValue(InputManager::feMoveRight, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_DPAD_RIGHT) || leftX > 0.5f ? 1.0f : 0.0f, false);

        // Start button for pause/menu
        SetButtonValue(InputManager::feStart, pInputMgr->IsXInputButtonPressed(m_controllerId, XINPUT_GAMEPAD_START) ? 1.0f : 0.0f, false);

        // Dispatch to mappables - always dispatch all buttons for proper camera control
        for(unsigned j = 0; j < Input::MaxMappables; j++)
        {
            if(mMappable[j])
            {
                ...
                {
                    mMappable[j]->DispatchOnButton(m_controllerId, b, &mButtonArray[b]);
                }
            }
        }
    }
