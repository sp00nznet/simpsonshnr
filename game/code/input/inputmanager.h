...
//        X,  //This one is conflicting with PS2
    ...
    enum eButtonMap
    {
        ...
        P1_KBD_Left,
        P1_KBD_Right,
        RightStickX,            // XInput right stick axis (for camera control)
        RightStickY,
    ...
    // PS3 DualShock 3 / SIXAXIS button mapping
    enum eButtonMap
    {
        Select,
        L3,
...
//        X,  //This one is conflicting with PS2
    ...
    // XInput support for modern Xbox controllers
    bool IsXInputConnected(int port) const;
    float GetXInputLeftStickX(int port) const;
    float GetXInputLeftStickY(int port) const;
    float GetXInputRightStickX(int port) const;
    float GetXInputRightStickY(int port) const;
    float GetXInputLeftTrigger(int port) const;
    float GetXInputRightTrigger(int port) const;
    bool IsXInputButtonPressed(int port, unsigned short button) const;
    void SetXInputVibration(int port, float leftMotor, float rightMotor);
#endif
