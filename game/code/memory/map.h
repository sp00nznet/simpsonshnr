...
typename Map< KeyClass, T >::const_iterator Map< KeyClass, T >::find( const KeyClass& key ) const
{
    ...
    findme.first = key;
    typename Map::const_iterator it = std::lower_bound( m_Elements.begin(), m_Elements.end(), findme );
    return it;
}
...
typename Map< KeyClass, T >::iterator Map< KeyClass, T >::find( const KeyClass& key )
{
    ...
    MapElement< KeyClass, T > findme;
    findme.first = key;
    typename Map::iterator it = std::lower_bound( m_Elements.begin(), m_Elements.end(), findme );
...
void Map< KeyClass, T >::insert( const KeyClass& key, const T& element )
{
    ...
    typename Map::iterator found = find( key );
    if( found != end() )
    {
...
{
    typename Map::iterator it = find( key );
    rAssert( it != end() );
    return (*it).second;
}
