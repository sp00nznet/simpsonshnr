...
#define INIT_MEM()  Memory::InitializeMemoryUtilities();PS3Platform::InitializeMemory();
void MemoryHackCallback() { INIT_MEM() };
#endif // RAD_PS3
...
// IMPORTANT: This MUST be thread-local to prevent race conditions when multiple
// threads call HeapManager::GetInstance() simultaneously. Without this, Thread A
// could set g_NoHeapRoute=true while Thread B is allocating, causing B's allocation
// to go to the wrong heap and corrupting memory when freed.
//
#ifdef RAD_PS3
// Use thread-local storage on PS3 to fix race condition
static __thread bool g_NoHeapRoute = false;
#else
...
#ifndef RAD_PS3
        // PS3 pddiPrimStream not available in this compile unit
...
//
    ...
    // XDR RAM: 256MB main memory (minus ~50MB system/OS overhead)
    // RSX VRAM: 256MB video memory (managed separately via PSGL)
    // Available for game: ~170MB after PSGL takes 32MB
    //
    #ifdef RAD_RELEASE
    const float HS_DEFAULT = 0.12f;  // For only very core FTech stuff
    #else
    const float HS_DEFAULT = 0.15f;   // For that plus debug comm stuff, etc
    #endif
    const float HS_TEMP = 0.4f;
    const float HS_PERSISTENT = 1.9f;
    const float HS_MUSIC = 0.2f;
    const float HS_AUDIO_PERSISTENT = 0.65f;
    // Note: HS_LEVEL not used on PS3 - we use dynamic allocation like PS2
    const float HS_LEVEL = 0.0f;  // Not used - PS3 uses dynamic default heap
    //FE Only
    const float HS_LEVEL_MOVIE = 2.0f;
    const float HS_LEVEL_AUDIO_FE = 0.05f;
    const float HS_LEVEL_FE = 14.0f;
    //In-game Only
    const float HS_LEVEL_ZONE = 11.5f;
    //const float HS_LEVEL_OTHER = 6.0f;
    const float HS_LEVEL_HUD = 1.7f;
    //const float HS_LEVEL_MISSION = 3.0f;
    const float HS_LEVEL_AUDIO_INGAME = 0.05f;
    //Minigame Only
    const float HS_MINIGAME_ZONE        = 3.0f;
    const float HS_MINIGAME_OTHER       = 7.7f;
    ...
    #ifndef RAD_RELEASE
    const float HS_DEBUG = 0.5f;
    const float HS_DEBUG_FIREWIRE = 0.4f;
    const float HS_SPECIAL = 0.5f;
    #endif
...
#if defined(RAD_PS3) && defined(ALL_DL_HEAPS)
    // PS3: Use fixed heap size - dynamic sizing doesn't work reliably with RPCS3
    // PS3 has 256MB total, minus ~80MB for system/PSGL, leaves ~176MB
    // Use a conservative fixed size that works reliably
    const unsigned int PS3_DEFAULT_HEAP_SIZE = 32 * 1024 * 1024;  // 32MB fixed

    rReleasePrintf("[PS3] Creating GMA_DEFAULT heap with fixed size: %d bytes (%.2f MB)\n",
                   PS3_DEFAULT_HEAP_SIZE, (float)PS3_DEFAULT_HEAP_SIZE / (1024.0f * 1024.0f));
    CreateHeap( GMA_DEFAULT, PS3_DEFAULT_HEAP_SIZE );
    IRadMemoryAllocator* defaultHeap = GetAllocator( GMA_DEFAULT );
    radMemoryRegisterAllocator( GMA_LEVEL_MOVIE,      RADMEMORY_ALLOC_DEFAULT, defaultHeap );
    radMemoryRegisterAllocator( GMA_LEVEL_FE,         RADMEMORY_ALLOC_DEFAULT, defaultHeap );
    radMemoryRegisterAllocator( GMA_LEVEL_ZONE,       RADMEMORY_ALLOC_DEFAULT, defaultHeap );
...
//    CreateHeap( GMA_MUSIC, static_cast< unsigned int >( HS_MUSIC * FUDGE ) );
...
#if !defined(RAD_RELEASE) && !defined(RAD_PS3)
    // The debug heap.
    ...
    // On PS3, GMA_DEBUG is registered to GMA_DEFAULT to save memory.
    //
    float debugSize;
    if( CommandLineOptions::Get( CLO_FIREWIRE ) )
    {
