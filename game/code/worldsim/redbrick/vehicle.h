...
public:
    ...
    float mStuckTimer;              // How long we've been stuck
    rmt::Vector mStuckCheckPosition; // Position when we started checking
    float mFlashTimer;              // Time remaining for flash effect
    bool mIsFlashing;               // Whether we're currently flashing
    static const float STUCK_THRESHOLD_SECONDS;  // How long before we teleport
    static const float FLASH_DURATION_SECONDS;   // How long to flash after teleport
