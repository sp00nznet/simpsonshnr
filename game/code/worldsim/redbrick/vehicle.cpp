...
const float Vehicle::STUCK_THRESHOLD_SECONDS = 3.0f;  // Time stuck before teleport
const float Vehicle::FLASH_DURATION_SECONDS = 5.0f;   // How long to flash after teleport
...
void Vehicle::ResetFlagsOnly(bool resetDamage)
{
    ...
    mStuckTimer = 0.0f;
    mStuckCheckPosition.Set(0.0f, 0.0f, 0.0f);
    mFlashTimer = 0.0f;
    mIsFlashing = false;
...
void Vehicle::Update(float dt)
{
    ...
    // Update stuck detection for player vehicle
    UpdateStuckDetection(dt);

}

//=============================================================================
// Vehicle::UpdateStuckDetection
//=============================================================================
// Description: Detects when player vehicle is stuck and teleports to safety
//
// Parameters:  (float dt)
//
// Return:      void
//
//=============================================================================
void Vehicle::UpdateStuckDetection(float dt)
{
    // Only apply to player-driven vehicles using physics locomotion
    if( !mUserDrivingCar || mLoco != VL_PHYSICS || mVehicleDestroyed )
    {
        return;
    }
    ...
    if( mIsFlashing )
    {
        ...
        if( mFlashTimer <= 0.0f )
        {
            ...
            if( mGeometryVehicle )
            {
                mGeometryVehicle->SetFadeAlpha( 255 );
            }
        }
        else
        {
            // Flash by alternating visibility (toggle every 0.15 seconds)
            int flashPhase = (int)(mFlashTimer / 0.15f);
            if( mGeometryVehicle )
            {
                mGeometryVehicle->SetFadeAlpha( (flashPhase % 2 == 0) ? 255 : 80 );
            }
        }
    }

    // Check if player is trying to drive (pressing gas or reverse)
    bool tryingToDrive = (mGas > 0.3f || mReverse > 0.3f);

    // Check if vehicle is barely moving despite input
    const float STUCK_SPEED_THRESHOLD = 2.0f; // km/h - very slow
    bool barelyMoving = (mSpeedKmh < STUCK_SPEED_THRESHOLD);

    // Additional check: position isn't changing
    rmt::Vector currentPos = GetPosition();
    float positionDelta = (currentPos - mStuckCheckPosition).MagnitudeSqr();
    const float STUCK_POSITION_THRESHOLD_SQR = 1.0f; // 1 meter squared
    bool positionStuck = (positionDelta < STUCK_POSITION_THRESHOLD_SQR);

    if( tryingToDrive && barelyMoving && positionStuck && !mIsFlashing )
    {
        mStuckTimer += dt;

        if( mStuckTimer >= STUCK_THRESHOLD_SECONDS )
        {
            // Player is stuck - teleport to safety
            ResetOnSpot( false, true );

            // Reset all velocities to prevent momentum issues
            mSimStateArticulated->ResetVelocities();
            mVelocityCM.Set(0.0f, 0.0f, 0.0f);
            mSpeed = 0.0f;
            mSpeedKmh = 0.0f;
            ...
            // Start flashing
            mIsFlashing = true;
            mFlashTimer = FLASH_DURATION_SECONDS;

            // Reset stuck timer
            mStuckTimer = 0.0f;
            mStuckCheckPosition = GetPosition();
        }
    }
    else
    {
        // Not stuck - reset timer and update check position
        mStuckTimer = 0.0f;
        mStuckCheckPosition = currentPos;
    }
}
