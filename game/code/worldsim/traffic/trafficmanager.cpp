...
#else
    const float TrafficManager::FADE_RADIUS = 100.0f;
    const float TrafficManager::CENTER_OFFSET = 15.0f;   // Reduced to spawn cars more around player
    const float TrafficManager::ADD_RADIUS = 50.0f;      // Reduced so cars spawn closer
    const float TrafficManager::REMOVE_RADIUS = 140.0f;
    const float TrafficManager::INITIAL_ADD_RADIUS = 80.0f;  // Reduced for faster initial spawn
#endif

const unsigned int TrafficManager::MILLISECONDS_BETWEEN_REMOVE = 200;
const unsigned int TrafficManager::MILLISECONDS_BETWEEN_ADD = 100;  // Faster respawn to fill gaps quickly
const unsigned int TrafficManager::MILLISECONDS_POPULATE_WORLD = 1500; // Faster initial population 
...
void TrafficManager::InitDefaultModelGroups()
{
    for( int i=0; i<TrafficManager::MAX_TRAFFIC_MODEL_GROUPS; i++ )
    {
        ...
        */
        ...
        // DEFAULTS - increased max instances to support more traffic
        mTrafficModelGroups[i].AddTrafficModel( "compactA", 5 );
        mTrafficModelGroups[i].AddTrafficModel( "pickupA", 5 );
        mTrafficModelGroups[i].AddTrafficModel( "sportsA", 5 );
        mTrafficModelGroups[i].AddTrafficModel( "SUVA", 5 );
    }
...
{
    static const unsigned int MAX_MILLISECONDS_OUT_OF_SIGHT_BEFORE_REMOVAL = 15000;
