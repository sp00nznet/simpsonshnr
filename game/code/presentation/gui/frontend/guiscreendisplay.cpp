...
MEMTRACK_PUSH_GROUP( "CGuiScreenDisplay" );
    ...
    // Add widescreen resolution strings to the resolution menu
    {
        Scrooby::Group* resGroup = pPage->GetGroup( "Resolution" );
        if( resGroup )
        {
            Scrooby::Text* resText = resGroup->GetText( "Resolution_Value" );
            if( resText )
            {
                // Cast to FeText to access AddHardCodedString
                FeText* feResText = dynamic_cast<FeText*>( resText );
                if( feResText )
                {
                    // Add widescreen resolutions (will be appended after existing strings)
                    feResText->AddHardCodedString( "1280x720" );
                    feResText->AddHardCodedString( "1366x768" );
                    feResText->AddHardCodedString( "1600x900" );
                    feResText->AddHardCodedString( "1920x1080" );
                    feResText->AddHardCodedString( "2560x1440" );

                    // Update the menu item to know about all resolution options
                    m_pMenu->SetSelectionValueCount( MENU_ITEM_RESOLUTION, feResText->GetNumOfStrings() );
                }
            }
        }
    }
