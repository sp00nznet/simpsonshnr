...
//            m_hnrSectors[ i ]->SetColour( HNR_METER_EMPTY_COLOUR );
    ...
    // When using XInput controller, show Xbox button name instead of action name
    if( GetInputManager()->IsXInputConnected(0) && m_actionButton != NULL )
    {
        m_actionButton->SetString( 0, "Y" );
    }
#else
